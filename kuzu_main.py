...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        #zero gradients computed over previous batches
        optimizer.zero_grad()
        output = model(data)
        #forward pass done, compute backward pass
        loss = F.nll_loss(output, target)
        loss.backward() #compute gradients
        optimizer.step() #updates gradients using learning rate and momentum
        #if epoch == 10: 
            #vis.line(np.array([loss.item()]), np.array([batch_idx]), win=vis_window, update='append')
        if batch_idx % 100 == 0:
...
def main():
    ...
    if list(net.parameters()):
        #experiment with learning rate and momentum
